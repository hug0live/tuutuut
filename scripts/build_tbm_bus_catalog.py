...
GTFS_PATH = Path(os.environ.get("TBM_GTFS_ZIP", "/tmp/tbm_gtfs.zip"))
OUTPUT_PATH = Path(__file__).resolve().parents[1] / "src" / "tbmBusCatalog.json"
SUPPORTED_ROUTE_TYPES = {"0", "3", "4"}
...
def main() -> int:
    ...
    with zipfile.ZipFile(GTFS_PATH) as archive:
        ...
        for route_id, route_row in sorted(
            routes.items(),
            key=lambda item: item[1]["route_short_name"],
        ):
