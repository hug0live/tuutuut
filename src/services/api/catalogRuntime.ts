...
export function createCatalogApi(catalogUrl: string, sourceLabel: string) {
  ...
  async function searchCatalogStops(query: string): Promise<Stop[]> {
  ...
  async function getCatalogLinesByStop(stopId: string): Promise<Line[]> {
  ...
  async function getCatalogLineStops(
    lineId: string,
    directionId?: string,
    anchorStopId?: string
  ): Promise<LineStop[]> {
