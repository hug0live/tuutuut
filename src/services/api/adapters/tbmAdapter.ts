...
export function normalizeTbmRealtimeLineReference(value: string): string {
  return normalizeIdentifier(value).replace(/^tbmline/, "");
}
...
export function createTbmAdapter(realtimeConfig: CityRealtimeConfig): TransportAdapter {
  const adapter: TransportAdapter = {
  source: {
    mode: "tbm",
    label: "Lignes TBM via Bus Tracker",
    detail:
      "Topologie GTFS locale TBM + positions vehicules reelles via Bus Tracker. Repli theorique si necessaire.",
    realtime: true
  },
